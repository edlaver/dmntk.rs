...
base64 --wrap=0 2_1.dmn >> data.json
echo -n '"}' >> data.json
...
# prepare JSON data file containing a model with the same name to be added to workspace
echo -n '{"content":"' > data.json
base64 --wrap=0 2_2.dmn >> data.json
echo -n '"}' >> data.json

# try to add the same model to workspace
curl -s -d '@data.json' -H "Content-Type: application/json" -X POST http://0.0.0.0:22022/definitions/add 2>&1
