...
/// Converts an URI into its RDNN equivalent.
///
/// Unless the input namespace URI is invalid or does not contain a domain name,
/// this function returns its RDNN equivalent. Domain name segments are reversed,
/// path segments order is preserved, all segments are joined with a dot `.` character.
///
/// Returns `None` for namespace URIs that can not be converted to RDNN.
///
/// # Examples
///
/// ```
...
pub fn to_rdnn(input: &str) -> Option<String> {
  ...
  let mut rdnn = domain.split('.').collect::<Vec<&str>>();
  rdnn.reverse();
  Some(format!("{}{}{}", rdnn.join("."), if segments.is_empty() { "" } else { "." }, segments))
}
