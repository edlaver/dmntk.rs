...
static MODEL_EVALUATOR: Lazy<Arc<ModelEvaluator>> = Lazy::new(|| build_model_evaluator(dmntk_examples::DMN_3_0056));
...
fn _0005(b: &mut Bencher) {
  ...
  assert_decision(&MODEL_EVALUATOR, invocable_name, &ctx, r#"null([core::modulo] division by zero)"#);
  b.iter(|| MODEL_EVALUATOR.evaluate_invocable(invocable_name, &ctx));
}
...
fn _0008(b: &mut Bencher) {
  ...
  assert_decision(&MODEL_EVALUATOR, invocable_name, &ctx, r#"null(expected 2 parameters, actual number of parameters is 3)"#);
  b.iter(|| MODEL_EVALUATOR.evaluate_invocable(invocable_name, &ctx));
}
...
fn _0010(b: &mut Bencher) {
  ...
  assert_decision(&MODEL_EVALUATOR, invocable_name, &ctx, r#"null(parameter 'divisor' not found)"#);
  b.iter(|| MODEL_EVALUATOR.evaluate_invocable(invocable_name, &ctx));
}
...
fn _0025(b: &mut Bencher) {
  ...
  assert_decision(&MODEL_EVALUATOR, invocable_name, &ctx, r#"1.1"#);
  b.iter(|| MODEL_EVALUATOR.evaluate_invocable(invocable_name, &ctx));
}
...
fn _0026(b: &mut Bencher) {
  ...
  assert_decision(&MODEL_EVALUATOR, invocable_name, &ctx, r#"3.4"#);
  b.iter(|| MODEL_EVALUATOR.evaluate_invocable(invocable_name, &ctx));
}
