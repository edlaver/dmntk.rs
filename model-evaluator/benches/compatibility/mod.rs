...
fn assert_decision(model_evaluator: &ModelEvaluator, namespace: &str, invocable_name: &str, input_data: &FeelContext, expected: &str) {
  let actual = model_evaluator.evaluate_invocable_by_name(namespace, invocable_name, input_data).to_string();
  assert_eq!(
    expected, actual,
...
fn assert_decision_service(model_evaluator: &ModelEvaluator, namespace: &str, invocable_name: &str, input: &str, expected: &str) {
  let actual = model_evaluator.evaluate_invocable_by_name(namespace, invocable_name, &context(input)).to_string();
  assert_eq!(
    expected, actual,
