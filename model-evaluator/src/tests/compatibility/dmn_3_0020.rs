...
fn _0001() {
  ...
  assert_decision(&MODEL_EVALUATOR, &MODEL_NAMESPACE, "Total Vacation Days", &ctx, r#"27"#);
}

#[test]
fn _0002() {
  let ctx = context(r#"{Age: 25,Years of Service: 5}"#);
  assert_decision(&MODEL_EVALUATOR, &MODEL_NAMESPACE, "Total Vacation Days", &ctx, r#"22"#);
}

#[test]
fn _0003() {
  let ctx = context(r#"{Age: 25,Years of Service: 20}"#);
  assert_decision(&MODEL_EVALUATOR, &MODEL_NAMESPACE, "Total Vacation Days", &ctx, r#"24"#);
}

#[test]
fn _0004() {
  let ctx = context(r#"{Age: 44,Years of Service: 30}"#);
  assert_decision(&MODEL_EVALUATOR, &MODEL_NAMESPACE, "Total Vacation Days", &ctx, r#"30"#);
}
