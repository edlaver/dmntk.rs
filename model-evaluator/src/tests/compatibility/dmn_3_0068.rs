...
fn _0081() {
  let ctx = context(r#"{}"#);
  assert_decision(&MODEL_EVALUATOR, &MODEL_NAMESPACE, "dt_duration_007", &ctx, r#"null(equal err 'P0D' =?= '0')"#);
}
...
fn _0089() {
  let ctx = context(r#"{}"#);
  assert_decision(&MODEL_EVALUATOR, &MODEL_NAMESPACE, "ym_duration_008", &ctx, r#"null(equal err 'P0Y' =?= '0')"#);
}
