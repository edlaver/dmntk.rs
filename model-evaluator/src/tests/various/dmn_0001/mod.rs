...
fn _0001() {
  ...
  assert_decision(&MODEL_EVALUATOR, "Selector", &ctx, r#""7a""#);
}

#[test]
fn _0002() {
  let ctx = context(r#"{ Days: 7, Bounds: { Min: 7, Max: 360 } }"#);
  assert_decision(&MODEL_EVALUATOR, "Selector", &ctx, r#""7b""#);
}

#[test]
fn _0003() {
  let ctx = context(r#"{ Days: 8, Bounds: { Min: 7, Max: 360 } }"#);
  assert_decision(&MODEL_EVALUATOR, "Selector", &ctx, r#""(7..360)""#);
}

#[test]
fn _0004() {
  let ctx = context(r#"{ Days: 360, Bounds: { Min: 7, Max: 360 } }"#);
  assert_decision(&MODEL_EVALUATOR, "Selector", &ctx, r#""360+""#);
}
