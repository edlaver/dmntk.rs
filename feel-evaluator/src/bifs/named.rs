...
fn bif_finished_by(parameters: &NamedParameters) -> Value {
  if let Some(((value1, 1), (value2, 2))) = get_param(parameters, &NAME_RANGE_1).zip(get_param(parameters, &NAME_RANGE_2)) {
    core::finished_by(value1, value2)
  } else if let Some(((value1, 1), (value2, 2))) = get_param(parameters, &NAME_RANGE).zip(get_param(parameters, &NAME_POINT)) {
    core::finished_by(value1, value2)
  } else {
